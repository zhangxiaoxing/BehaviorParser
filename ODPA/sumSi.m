...
chr2=data.chr2;
tasks={'baseline','delay5s','delay8s','delay12s','firstOdor','secondOdor','bothOdor','responseDelay'};
tags={'Baseline','Delay 5s','Delay 8s','Delay 12s','First Odor','Second Odor','Both Odor','Response Delay'};
tCount=size(tasks,2);
dp=nan(tCount,1);
df=dp;
semp=dp;
...
%         d=(optoOn-optoOff)./optoOff*100;
    ...
    function plotOne(data,sem,p,label)
        hold on;
        h=bar(1:tCount,data,barw);
        h.FaceColor='w';
        h.LineWidth=1;
        
        errorbar(1:length(data),data,sem,'k.');
