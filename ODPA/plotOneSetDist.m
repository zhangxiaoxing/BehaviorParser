function plotOneSet(data,measure,tag,forppt)
path(path,'D:\behavior\reports\z');


ctrlOff=cell2mat(data(strcmpi('none',data(:,10)) & strcmpi('ctrl',data(:,6)),measure));
ctrlOn=cell2mat(data(strcmpi('distr',data(:,10)) & strcmpi('ctrl',data(:,6)),measure));
optoOff=cell2mat(data(strcmpi('none',data(:,10)) & strcmpi('ChR2',data(:,6)),measure));
optoOn=cell2mat(data(strcmpi('distr',data(:,10)) & strcmpi('ChR2',data(:,6)),measure));
if (~forppt)
...
% plotPopulation();
    ...
    function plotParallel()
        ...
        plot(repmat(place(1:2),size(ctrlOff,1),1)',[ctrlOff ctrlOn]','k-','Color',zGetColor('r0'));
        plot(repmat(place(3:4),size(optoOff,1),1)',[optoOff optoOn]','k-','Color',zGetColor('r0'));
        
        plot(place(1),ctrlOff,'ko','MarkerSize',4,'MarkerFaceColor','w');
        plot(place(2),ctrlOn,'ko','MarkerSize',4,'MarkerFaceColor','w','MarkerEdgeColor',zGetColor('r0'));
        plot(place(3),optoOff,'ko','MarkerSize',4,'MarkerFaceColor','k');
        plot(place(4),optoOn,'ko','MarkerSize',4,'MarkerFaceColor',zGetColor('r0'),'MarkerEdgeColor',zGetColor('r0'));
...
%         end
        ...
        set(gca,'XTick',place,'XTickLabel',{'No','D+','No','D+'},'YColor','k','XColor','k','FontSize',10);
...
%         text(place(4)+0.7,topY(1)-5*sigDY,'On','Color','k','Rotation',0,'FontSize',10,'HorizontalAlignment','right','FontName','Helvetica');
    ...
    function plotMean(data,position)
        edgeColors={'k',zGetColor('r0'),'k',zGetColor('r0')};
        faceColors={'w','w','k',zGetColor('r0')};
        mLength=1.5;
        dd=0.5;
        semLength=0.25;
        dh=1;
